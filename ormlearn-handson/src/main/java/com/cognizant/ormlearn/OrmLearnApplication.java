...
public class OrmLearnApplication {
	...
	@Bean
	CommandLineRunner testFindStock() {
		return args -> {
			logger.info("START... for findStock");
			stockService.findStock("GOOGL").forEach(c -> logger.info("{}", c));
			logger.info("END... for findStock");
		};
	}

	@Bean
	CommandLineRunner testFindFBStock() {
		return args -> {
			logger.info("START... for findFBStock");
			List<Stock> stockInSep19 = stockService.findFBStock("FB",
			...
			logger.info("END... for findFBStock");
		};
	}

	@Bean
	CommandLineRunner testFindGoogleStock() {
		return args -> {
			logger.info("START... for findGoogleStock");
			stockService.findGoogleStock("GOOGL", 1250).forEach(c -> logger.info("{}", c));
			logger.info("END... for findGoogleStock");
		};
	}

	@Bean
	CommandLineRunner testFindTop3VolStock() {
		return args -> {
			logger.info("START... for findTop3VolStock");
			stockService.findTop3VolStock().forEach(c -> logger.info("{}", c));
			logger.info("END... for findTop3VolStock");
		};
	}

	@Bean
	CommandLineRunner testFindLowest3FLXStock() {
		return args -> {
			logger.info("START... for findLowest3NLFXStock");
			stockService.findLowest3NFLXStock("NFLX").forEach(c -> logger.info("{}", c));
			logger.info("END... for findLowest3NFLXStocks");
		};
	}
	...
	CommandLineRunner testAddEmployee() {
		return args -> {
			...
			Employee employee = Employee.builder().name("def").salary(750000.00).permanent(false)
